...
$allowedType = "House";
$googleGeoCodingApiKey = "AIzaSyDf1zH207xn4tI_JE9wrAlydz0_TVBThtg";
...
function getListTypeOld($dat)
{
	return $dat['listing']['property_type'];
}
...
{
	$url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=".$lat.",".$long."&key=".$googleGeoCodingApiKey;
	$dom = getDom($url, true);
	$dat = json_decode($dom, true);
	if($dat['status'] == "OK")
	{
...
{
	global $allowedType;
	if (strpos($type, $allowedType) !== FALSE) return true;
}

function IsUnique($id)
{
	global $successes;
	if (!in_array("https://www.airbnb.com/rooms/".$id, $successes)) return true;
}